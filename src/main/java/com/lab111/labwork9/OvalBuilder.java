...
public class OvalBuilder implements Builder {
    private Action action;

    @Override
    public void buildShape(int[] data) {
        try {
            action.pressButton(3);
        } catch (Exception e) {
            System.out.println("Nooo");
        }

        action.draw.setFlag(false);
        action.shapes.get(action.shapes.size() - 1).draw(new Rectangle(data[0], data[1], data[2], data[3]), 0);
