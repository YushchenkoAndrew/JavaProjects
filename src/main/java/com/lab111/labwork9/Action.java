...
//                draw.setCheckPressingFlag();
            ...
            case 10: {       //buttons's event open file
                director.setAction(this);
                fileReader = new FileReader(gui.textFields[1].getText());
                fileReader.setAction(this);
                fileReader.readFile();
                movingFlag = false;
                fillShapeInColor = false;
                flagCopy = false;
    ...
    public void mouseReleased() {
        ...
        if (mementos[mementoIndex] != null && !movingFlag && !fillShapeInColor && gui.indexButton != 54) {
    ...
    boolean drawFirstElement = false;

    public void mousePressedMove() {

        if (drawFirstElement) {
            draw.setFlag(false);
            drawFirstElement = false;
        }
        if (shapes.size() > 0)
